...
def ensure_pre_gui_init() -> None:
    ...
    if sys.platform == "win32":
        from app_platform.windows_capture import ensure_windows_dpi_awareness
...
def enumerate_windows(min_width: int = 80, min_height: int = 80) -> List[WindowEntry]:
    if sys.platform == "win32":
        from app_platform.windows_capture import enumerate_windows as _ew

        return _ew(min_width=min_width, min_height=min_height)
    if sys.platform == "darwin":
        from app_platform.darwin_capture import enumerate_windows as _ew
...
def make_window_capture(window_id: int) -> BgrWindowCapture:
    ...
    """
    if sys.platform == "win32":
        from app_platform.windows_capture import WindowCapture

        return WindowCapture(window_id)
    if sys.platform == "darwin":
        from app_platform.darwin_capture import MacWindowCapture
